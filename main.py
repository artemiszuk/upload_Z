...
from plugins.vars import messageobj, Var
'''tdict = dict()
Var.upload_as_doc = dict()
Var.q_link = dict()'''
...
async def upload(message, filepath, user_id):
  c_time= time.time()
  if user_id not in Var.upload_as_doc : Var.upload_as_doc[user_id] = True
  ...
  if(user_id in Var.tdict):
    if Var.upload_as_doc[user_id] == False  and (exten == '.mp4' or exten == '.mkv'):
      ...
      await app.send_video(user_id, filepath,supports_streaming=True,caption=filename,thumb=str(Var.tdict[user_id]),duration=int(float(mydict['duration'])),width=int(mydict['width']),height = int(mydict['height']),progress=progress_for_pyrogram,progress_args=("Upload Status: \n",message,c_time))
    else:
      await app.send_chat_action(user_id, "upload_document")
      await app.send_document(user_id, filepath,caption=filename,thumb=str(Var.tdict[user_id]),progress=progress_for_pyrogram,progress_args=("Upload Status: \n",message,c_time))
  else:
    if Var.upload_as_doc[user_id] == False  and (exten == '.mp4' or exten == '.mkv'):
      mydict = await get_details(filepath)
      await app.send_chat_action(user_id, "upload_video")
...
async def button(client, cmd: CallbackQuery):
  ...
  if "help" in cb_data :
    ...
    if Var.upload_as_doc[user_id]: Var.upload_as_doc[user_id] = False
    else : Var.upload_as_doc[user_id] = True
    text = "**⚙TOGGLE MENU:**\n[__Click to change__]\n\nVideo File Will be Uploaded as "
    if Var.upload_as_doc[user_id]: text_append = "Document 📁"
...
async def link(client, message,unzipflag = False):
  user_id = message.chat.id
  if user_id not in Var.q_link : Var.q_link[user_id] = []
